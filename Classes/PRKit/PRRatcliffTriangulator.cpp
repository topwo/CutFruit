...
CCArray* PRRatcliffTriangulator::triangulateVertices(cocos2d::CCArray* vertices){
	...
	CCArray* triangulatedPointsArray = CCArray::createWithCapacity(triangulatedPointCount);
	for(int i = 0; i < triangulatedPointCount; i++){
		CCString* triangulatedPointValue = CCString::createWithFormat("{%f, %f}", triangulatedPoints[i].GetX(), triangulatedPoints[i].GetY());
